...
class GuardrailRunner:
    ...
    def run_full_pipeline(
        ...
        retrieval_valid, retrieval_issues = (
            self.retrieval_validator.validate(
                retrieval_results
            )
        )
        ...
        if (
            not retrieval_valid
            and self.policies.min_retrieval_confidence > 0.3
        ):
            return GuardrailPipelineResult(
                input_valid=True,
                retrieval_valid=False,
                ...
                final_safe_output=(
                    "Insufficient or unreliable evidence found."
                ),
            )
        ...
        if raw_llm_output:
            ...
            final_output = (
                "No answer generated due to risk constraints."
            )
            output_valid = False
